...
<script type="text/javascript">
  ...
  $(function(){
	...
	function del(obj,id){
		...
		$.post("/admin/article/update",{id:id,deleted:deleted},function(result){
	    	if(result.code==0){
	        //	alert("操作成功");
	...
	//分页
		...
		//修改状态
		function update1(obj,id){
			  //0:正常 1:停用
			  //如果当前状态为正常,则改为停用.如果是停用则改为正常
			  var hot =$(obj).text()=="否"?"1":"0";
			  $.post("/admin/article/update",{id:id,hot:hot},function(result){
		        if(result.code==0){
			...
			<tbody>
				<c:forEach items="${info.list}" var="a" varStatus="i">
					<tr>
						...
						<td>
							<c:if test="${a.hot==0 }">
								<button type="button" class="btn btn-info" onclick="update1(this,${a.id})">否</button>
							</c:if>
							<c:if test="${a.hot==1 }">
								<button type="button" class="btn btn-success" onclick="update1(this,${a.id})">是</button>
						...
						<td>
							<c:if test="${a.deleted==0 }">
								<button type="button" class="btn btn-warning" onclick="del(this,${a.id})">正常</button>
							</c:if>
							<c:if test="${a.deleted==1 }">
								<button type="button" class="btn btn-danger" onclick="del(this,${a.id})">已删除</button>
							</c:if>
						</td>
						<td>
							<button class="btn btn-primary" data-toggle="modal" data-target="#myModal" onclick="detail(${a.id})">详情</button>
						</td>
					</tr>
				</c:forEach>
			</tbody>
